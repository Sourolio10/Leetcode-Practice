def dfs(row, col, vrow, vcol, graph, vis,n,m):
    ...
    for i in range(4):
        ...
        if nrow>=0 and nrow<n and ncol>=0 and ncol<m and vis[nrow][ncol] == 0 and graph[nrow][ncol] == 'O':
...
def surrounded(graph):
    ...
    for j in range(m):
        if vis[0][j] == 0 and graph[0][j] == 'O':
            dfs(0, j, vrow, vcol, graph, vis,n,m)

        if vis[n-1][j] == 0 and graph[n-1][j] == 'O':
            dfs(n-1, j, vrow, vcol, graph, vis,n,m)

    for i in range(n):
        if vis[i][0] == 0 and graph[i][0] == 'O':
            dfs(i, 0, vrow, vcol, graph, vis,n,m)

        if vis[i][m-1] == 0 and graph[i][m-1] == 'O':
            dfs(i, m-1, vrow, vcol, graph, vis,n,m)

    for i in range(n):
        for j in range(m):
            if vis[i][j] == 0 and graph[i][j] == 'O':
                graph[i][j] = 'X'
